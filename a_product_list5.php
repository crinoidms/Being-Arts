...
<?php  include __DIR__ .'/__html_nav.php'; ?>
        ...
        <header class="flex between">
            ...
            <ul class="store_nav flex">
                <?php /* while($m_row = $m_result->fetch_assoc()): ?>
                    <li><a type="button" class="<?= $cate==$m_row['sid']? 'active' : ''?>" href="?cate=<?= $m_row['sid']?>"><?= $m_row['name']?></a></li>
                <?php endwhile; */ ?>

                <?php  foreach($menu as $item): ?>  <!-- menu-->
                    <li><a type="button" class="" href="?cate=<?php $item['sid'] ?>"><?= $item['name'] ?></a>
                        <?php if(! empty($item['sub'])): ?>
                        <ul class="sub_store_nav flex">
                            <?php foreach($item['sub'] as $i): ?>
                            <li><a href="?cate=<?= $i['sid'] ?>"><?= $i['name'] ?></a></li>
                            <?php endforeach; ?>
                        </ul>
                        <?php endif; ?>
                    </li>
            ...
            <div class="filter">
                <div class="school">
                    <ul class="flex ">
                        ...
                        <?php while($fs_row = $fs_result->fetch_assoc()): ?>
                            <input type="checkbox" id="school<?= $fs_row['sid']?>" name="school" checked>
                ...
                <ul class="flex color_filter">
                    ...
                    <?php while($fc_row = $fc_result->fetch_assoc()): ?>
                        <input type="checkbox" id="color<?= $fc_row['sid']?>" name="school" checked>
            ...
            <?php /*  <script>
                ...
                <div class="product" data-sid="<?= $row['sid'] ?>">
                    ...
                    <div class="info">
                        ...
                        <a type="button" class="love_btn"><img src="./imgs/icon/favorite_30px.svg" alt="購物袋"  title="購物袋"></a>
                    </div>
                </div>
                <?php endwhile; ?>
            </div>
        ...
        <div class="pagination">
                <ul class="page flex">
                    ...
                    if($i>=1 and $i<=$t_pages):
                        ?>
                        <li class="<?= $i==$page ? 'active' : '' ?>"><a href="?cate=<?= $cate ?>&school=<?= $school?>&page=<?= $i ?>"><?= $i ?></a></li>
                    <?php endif; ?>
                    <?php endfor; ?>
                    <li>>></li>
                </ul>
